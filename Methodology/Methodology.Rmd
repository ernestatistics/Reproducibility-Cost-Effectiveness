...
aux.function.url <-'https://rawgit.com/ernesto-up/Reproducibility-Cost-Effectiveness/master/Auxiliary%20Functions/auxiliary-functions.R'
temp <- getURL(aux.function.url,followlocation = TRUE, cainfo = system.file("CurlSSL", "cacert.pem", package = "RCurl"))
source(aux.function.url)
...
conapo <- getURL(url)                
conapo <- read.csv(textConnection(conapo))
#conapo <- read.csv('Data/Conapo-Projections.csv')
```
...
githubURL <-'https://github.com/ernesto-up/Reproducibilidad-BrCa-Cost-Benefit/blob/master/Datos/SeguroPopular.RData?raw=true'
load(url(githubURL))
```
...
```{r,warning=FALSE,message=FALSE}
datos$tiempo.vida.years <- as.numeric((datos$tiempo.vida)/365.25)
data.by.stage <- split(datos,datos$etapa.clinica.2)
```
...
life.tables <- lapply(data.by.stage,function(x){
                            time.life.years.redo <-floor(x$tiempo.vida.years)
                            data.table <- data.frame(time.life.years.redo,x$censura.vida)
                            die<-gsummary(data.table,sum,groups=time.life.years.redo)
                            total<-gsummary(data.table, length, groups=time.life.years.redo)
                            ltab.data<-cbind(die[,1:2], total[,2])
                            rm(time.life.years.redo)
                            attach(ltab.data)
                            lt=length(time.life.years.redo)
                            time.life.years.redo[lt+1]=NA
                            nevent = x.censura.vida
                            nlost = total[,2] - x.censura.vida
                            n = nrow(x)
                            mytable <- lifetab(time.life.years.redo, n, nlost, nevent)
                            detach(ltab.data)
                            mytable[,1:5]
                            })
```
...
#save.image("C:/Users/epidemiologia/Desktop/ernesto/CaMa/datos/parametros deteccion.RData")
```
...
#githubURL <- "https://github.com/ernesto-up/BrCa-Cost-Benefit/raw/master/Parametros-cost-Beneficio.RData"
#githubURL <- "https://github.com/ernesto-up/BrCa-Cost-Benefit/raw/master/Parametros-Sensibilidad-r-0.RData"
#githubURL <- "https://github.com/ernesto-up/BrCa-Cost-Benefit/raw/master/Parametros-Sensibilidad-Taghipour.RData"

load(url(githubURL))

load('/Users/Ernesto/Documents/INCAN/BrCa-Cost-Benefit/')
```

We define all the screening percentages for each age group: 
...
We define the periodicity of the programs:
...
We create a data frame with all the information for each intervention and run the simulations. We will store all the results in the same data frame.

```{r}
# Creamos data frame de intervenciones:

interventions <- data.frame(expand.grid(list = list(periodicity = periodicity,porc.25.40 = porc.25.40, porc.40.45 = porc.40.45,
...
for(i in 1:dim(intervenciones)[1])
{
  percentage <- data.frame(percentage = as.numeric(interventions[i,2:9]),age.group=age.group.data.frame)
  periodicity <- as.numeric(interventions[i,'periodicity'])
  ...
  interventions[i,10:13] <- results
}

save.image("~/Documents/INCAN/BrCa-Cost-Benefit/resultados-simulacion-sensibilidad-r-0-4-Sept.RData")
```
